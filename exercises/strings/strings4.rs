...
fn main() {
    string_slice("blue");  // 字符串字面值，类型为 &str
    string("red".to_string());  // to_string() 转换为 String
    string(String::from("hi"));  // String::from() 创建 String
    string("rust is fun!".to_owned());  // to_owned() 生成 String
    string("nice weather".into());  // into() 推断为 String 类型
    string(format!("Interpolation {}", "Station"));  // format! 宏返回 String
    string_slice(&String::from("abc")[0..1]);  // 字符串切片操作，返回 &str
